...
typedef struct {
	...
	cUInt32 imr0Val;
	cUInt32 imr1Val;
#ifdef CONFIG_KEYBOARD_BCM_WAKEUP_KEYS		// 2013.01.28 Jeeon.park	for setting wakeup keys during suspend(sleep) state
	cUInt32 wkimr0Val;		// Wakeup Key Interrupt Masking Register 0
	cUInt32 wkimr1Val;		// Wakeup Key Interrupt Masking Register 1
#endif
				    ...
				    CHAL_KEYPAD_INTERRUPT_EDGE_t edge)
{
 ...
 void chal_keypad_set_interrupt_reg(CHAL_HANDLE handle, cUInt32 row, cUInt32 column, CHAL_KEYPAD_KEYTYPE_t keytype)
 {
	 cUInt32 tempReg;
	 CHAL_KEYPAD_DEVICE_t *pKpdDev = (CHAL_KEYPAD_DEVICE_t *) handle;
 
	 tempReg = ( 1 << column ) << ((row%4) * 8);

	 if (keytype==CHAL_KEYPAD_IDLE_KEY)
	 {
		 if (row < 4)
			 pKpdDev->imr0Val |= tempReg;
		 else
			 pKpdDev->imr1Val |= tempReg;
	 }
	 else if(keytype==CHAL_KEYPAD_WAKEUP_KEY)
	 {
		 if (row < 4)
			 pKpdDev->wkimr0Val |= tempReg;
		 else
			 pKpdDev->wkimr1Val |= tempReg;
	 }
 
	 return;
 }
 
 void chal_keypad_clear_interrupt_reg(CHAL_HANDLE handle, CHAL_KEYPAD_KEYTYPE_t keytype)
 {
	 CHAL_KEYPAD_DEVICE_t *pKpdDev = (CHAL_KEYPAD_DEVICE_t *) handle;
 ...
 void chal_keypad_set_interrupt_mask(CHAL_HANDLE handle, CHAL_KEYPAD_KEYTYPE_t keytype)
 {
	 CHAL_KEYPAD_DEVICE_t *pKpdDev = (CHAL_KEYPAD_DEVICE_t *) handle;
	 ...
	 {
		 BRCM_WRITE_REG(pKpdDev->baseAddr, KEYPAD_KPIMR0, pKpdDev->wkimr0Val );
		 BRCM_WRITE_REG(pKpdDev->baseAddr, KEYPAD_KPIMR1, pKpdDev->wkimr1Val );  
	 }
